    ...
    'margin-right': '10px'});
        ...
        $(document).on("click","#mp_t_and_c", function()
		{
            ...
            .then(function (modal) {
                ...
                var $modal = $(modal);
                $('#mp_t_and_c_modal  .mp_wrap_content').append(mp_t_and_c)
            });
        ...
        function validate_profile_url($url_value){
            ...
            $('.oe_login_buttons :button').addClass('disabled');
            ...
            if(profile_url != '' && /^[0-9]+$/.test(profile_url)) {
              $profile_div.addClass('has-error');
              $('#profile_url_error').html(_t('URL handler must contain atleast 1 letter(a-z).Only alphanumeric([0-9],[a-z]) and some special characters(-,_) are allowed.'));
              $('#profile_url').addClass('invalide_url');
              $('#mp_terms_conditions').attr('checked',false);
              $('#profile_url_error').show();
            }
            else if (profile_url != '' && !profile_url.match(/^[a-zA-Z0-9-_]+$/)){
                $profile_div.addClass('has-error');
                $('#profile_url_error').html(_t('Only alphanumeric([0-9],[a-z]) and some special characters(-,_) are allowed, Spaces are not allowed.'));
                $('#profile_url').addClass('invalide_url');
                $('#mp_terms_conditions').attr('checked',false);
                $('#profile_url_error').show();
            }
                ...
                if(profile_url != '' && profile_url.match(/^[-_][a-zA-Z0-9-_]*$/) || profile_url.match(/^[a-zA-Z0-9-_]*[-_]$/)){
                    $profile_div.addClass('has-error');
                    $('#profile_url_error').html(_t('Special characters are not allowed in begining or at the last of the string.'));
                    $('#profile_url').addClass('invalide_url');
                    $('#mp_terms_conditions').attr('checked',false);
                    $('#profile_url_error').show();
                }
    				...
    				{
                        if(vals && profile_url != ''){
                            if($('.oe_login_buttons :button').hasClass('disabled')){
                            $('.oe_login_buttons :button').removeClass('disabled');
                            }
                            ...
                            if(profile_url != ''){
                                $('#profile_url_error').html(_t('Sorry, this profile url is not available.'));
                            }
                            else{
                                $('#profile_url_error').html(_t('Please Enter Your Profile URL.'));
                            }
                            $('#profile_url').addClass('invalide_url');
                            $('#profile_url_error').show();
                            $('#mp_terms_conditions').attr('checked',false);
