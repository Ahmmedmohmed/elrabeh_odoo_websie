...
class ResConfigSettings(models.TransientModel):
    ...
    enable_notify_admin_on_product_approve_reject = fields.Boolean(
        string="Enable Notification Admin On request for Product Approval")
    enable_notify_seller_on_product_approve_reject = fields.Boolean(
        string="Enable Notification Seller On Product Approve/Reject")
    enable_notify_seller_on_new_order = fields.Boolean(string="Enable Notification Seller On New Order Confirm")
    
    enable_notify_admin_on_no_qty= fields.Boolean(string="Enable Notification Admin On No. Of Quantity")
    ...
    notify_admin_on_product_approve_reject_m_tmpl_id = fields.Many2one(
        "mail.template", string="Mail Template to Notify Admin On request for Product Approval", domain="[('model_id.model','=','product.template')]")
    ...
    notify_seller_on_new_order_m_tmpl_id = fields.Many2one(
        "mail.template", string="Mail Template to Notify Seller On New Order Confirm", domain="[('model_id.model','=','sale.order')]")
    
    notify_admin_on_no_qty = fields.Many2one(
        "mail.template", string="Mail Template to Notify Seller on low sale order", domain="[('model_id.model','=','sale.order.line')]")
    ...
    enable_marketplace = fields.Boolean(related="website_id.enable_marketplace", string='Enable Marketplace', readonly=False)
    ...
    def set_values(self):
        ...
        self.env['ir.default'].sudo().set('res.config.settings', 'enable_notify_admin_on_no_qty', self.enable_notify_admin_on_no_qty)
        self.env['ir.default'].sudo().set('res.config.settings', 'notify_admin_on_no_qty', self.notify_admin_on_no_qty.id)
    ...
    def get_values(self):
        ...
        enable_notify_admin_on_no_qty = self.env['ir.default'].get('res.config.settings', 'enable_notify_admin_on_no_qty')
        notify_admin_on_no_qty = self.env['ir.default'].get('res.config.settings', 'notify_admin_on_no_qty')
        ...
        res.update(
            ...
            enable_notify_admin_on_no_qty = enable_notify_admin_on_no_qty,
            ...
            notify_admin_on_no_qty = notify_admin_on_no_qty,
