...
class ProductTemplate(models.Model):
    ...
    marketplace_seller_id = fields.Many2one(
        "res.partner", string="Seller", default=lambda self: self.env.user.partner_id.id if self.env.user.partner_id and self.env.user.partner_id.seller else False, copy=False, tracking=True, help="If product has seller then it will consider as marketplace product else it will consider as simple product.")
    ...
    def _get_pending_qty_request(self):
        """ Return True if there is any pending record for same product"""
        for obj in self:
    ...
    def mp_action_view_sales(self):
        """ Method used in product price comparison module """
        self.ensure_one()
    ...
    def approved(self):
        ...
        for obj in self:
            if not obj.marketplace_seller_id:
                raise UserError(_("Marketplace seller id is not assign to this product."))
            if obj.marketplace_seller_id.state != "approved":
                raise UserError(_("Marketplace seller of this product is not approved."))
        ...
        for template_obj in self:
            ...
            if len(self) == 1:
                if template_obj.mp_qty < 0:
                    raise UserError(_('Initial Quantity can not be negative'))
            if not location_id:
                raise UserError(_("Product seller has no location/warehouse."))
            if template_obj.mp_qty > 0:
                vals = {
    ...
    def _get_combination_info(self, combination=False, product_id=False, add_qty=1, pricelist=False, parent_combination=False, only_template=False):
        ...
        """Return info about a given combination and add extra values"""
