...
class ResPartner(models.Model):
    ...
    def get_mention_suggestions(self, search, limit=8, channel_id=None):
        ...
        if not self.env.user.has_group ('odoo_marketplace.marketplace_officer_group'):
           return []
        return super(ResPartner,self).get_mention_suggestions(search=search,limit=limit,channel_id=channel_id)
    ...
    #deny reason field
    deny_reason = fields.Char("Deny Reason", copy=False)
    ...
    def on_change_payment_assest(self):
        if self.commission < 0 or self.commission >= 100:
            raise UserError(_("Commission should be greater than 0 and less than 100."))
    ...
    def approve(self):
        ...
        if self.seller:
            self.state = "approved"
            self.change_seller_group_and_send_mail()
    ...
    def change_seller_group(self, set_to_group=None):
        ...
        if not login_user_obj.has_group('odoo_marketplace.marketplace_officer_group'):
            _logger.warning(_("warnings!! You are not an autorized user to change seller account access. Please contact your administrator. "))
